var typingText, inputField, mainMistakeTag, timeTag, wpmTag, cpmMainTag, resetBtn, accMainTag;
...
let timer, charIndex = 0, mistakes = 0, maxTime = 1000, timeLeft = maxTime, isTyping = false, canBackSpace = true;
var wpm = 0, cpm = 0, accuracy = 0, rawwpm = 0, totalChars = 0, wordsIndex = 0, currentCharIndex = 0, currentChars = 0;
let lastSpaceIndex = -1;

document.addEventListener("DOMContentLoaded", function () {
   
    typingText = document.querySelector(".text-container p"),
    ...
    function randomParagraph() {
        ...
        //focus on inout when keydown or click
    ...
    function initTyping() {
        ...
        inputField.addEventListener("keydown", event => {
            if (event.ctrlKey && event.code === 'Backspace' && charIndex != 0) {
                if ((event.code === "Backspace" && chars[charIndex - 1].innerText === " ") && !chars[charIndex - 1].classList.contains("warning")) {
                    event.preventDefault();
                }
                else {
                    if (charIndex != 0) {
                        for (let i = charIndex - 1; i >= charIndex - currentChars; i--) {
                            if (chars[i]) {
                        ...
                        charIndex -= currentChars;
                        currentChars = 0;
                    }
                }
            }
            else if ((event.code === "Backspace" && chars[charIndex - 1].innerText === " ") && (!chars[charIndex - 1].classList.contains("warning"))) {
                event.preventDefault();
            }
        });
        if (charIndex < chars.length - 1 && timeLeft > 0) {
            ...
            if (typeChar === " " && chars[charIndex].classList.contains("warning")) {
                chars[charIndex].classList.remove("warning");
            }
            if (typeChar == null) {
                ...
                if (chars[charIndex].innerText.trim() === typeChar.trim()) {
                    console.log("equal aya");
                    if (chars[charIndex].innerText === " " && typeChar === " ") {
                        currentChars = 0;
                    }
                    ...
                    if (chars[charIndex].innerText.trim() === "" && typeChar.trim() != "") {
                        chars[charIndex].classList.add("warning");
                        charIndex++;
                        initTyping();
                    }
                    mistakes++;
    ...
    //     inputField.addEventListener("keydown", event => {
    //         if (event.ctrlKey && event.code === 'Backspace' && charIndex != 0) {
    //             // Get the current word's starting and ending indices
    //             let start = charIndex;
    //             while (start > 0 && chars[start - 1].innerText !== " ") {
    //                 start--;
    //             }
    //             let end = charIndex;
    //             while (end < chars.length && chars[end].innerText !== " ") {
    //                 end++;
    //             }

    //             if (charIndex === end && start !== end) {
    //                 // The cursor is at the end of a word, so delete the entire word
    //                 for (let i = charIndex - 1; i >= start; i--) {
    //                     if (chars[i]) {
    //                         if (chars[i].classList.contains("correct")) {
    //                             chars[i].classList.remove("correct");
    //                         } else {
    ...
    //                 // The cursor is not at the start of a word, so delete a single character
    //                 if ((event.code === "Backspace" && chars[charIndex - 1].innerText === " ") && !chars[charIndex - 1].classList.contains("warning")) {
    //                     event.preventDefault();
    //                 } else {
    //                     for (let i = charIndex - 1; i >= charIndex - currentChars; i--) {
    ...
    //             chars[charIndex].classList.add("active");
    //         } else {
    //             // When there are no more characters to type, clear the input field and stop the timer.
    //             inputField.value = "";
    //             clearInterval(timer);
    //             return;
    //         }
    ...
    randomParagraph();
    document.addEventListener("input", initTyping);
